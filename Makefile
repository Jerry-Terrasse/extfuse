...
LINUXINCLUDE := $(subst "-I.",-I$(KDIR),$(LINUXINCLUDE))
...
# tprogs-y := libextfuse.so

# Libbpf dependencies
LIBBPF_SRC = $(PWD)/libbpf/src
...
all:
	$(MAKE) -C /lib/modules/`uname -r`/build M=$(CURDIR) BPF_SAMPLES_PATH=$(CURDIR)
...
$(obj)/%.o: $(src)/%.c
	@echo "  CLANG-bpf " $@
	$(Q)$(CLANG) $(NOSTDINC_FLAGS) \
		$(subst ./,$(KDIR)/,$(LINUXINCLUDE)) \
		$(BPF_EXTRA_CFLAGS) \
		-I$(obj) -I$(KDIR)/tools/testing/selftests/bpf/ \
		-I$(LIBBPF_SRC) \
		...
		-I$(KDIR)/samples/bpf/ -include asm_goto_workaround.h \
		-I$(obj)/include \
